...
class _SimpleAddShopActivityState extends State<SimpleAddShopActivity> {
  ...
  List<String> _selectedDays = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];

  // Predefined options
  final List<BusinessTypeOption> _businessTypes = [
    BusinessTypeOption("Car Wash", Icons.local_car_wash, Colors.blue),
    BusinessTypeOption("Auto Repair", Icons.build, Colors.orange),
    BusinessTypeOption("Detailing", Icons.cleaning_services, Colors.green),
    BusinessTypeOption("Tire Service", Icons.directions_car, Colors.purple),
  ];
  ...
  Widget _buildBusinessTypeCard(BusinessTypeOption type) {
    ...
    return Container(
      ...
      child: InkWell(
        onTap: () {
          setState(() {
            _selectedBusinessType = type.name;
          });
        },
    ...
    try {
      ...
      for (Location location in locations.take(5)) { // Limit to 5 results
        try {
          ...
          suggestions.add({
            'name': query,
            'formatted_address': 'Location: ${location.latitude.toStringAsFixed(4)}, ${location.longitude.toStringAsFixed(4)}',
            'geometry': {
              'location': {
                'lat': location.latitude,
                'lng': location.longitude,
  ...
  String _formatReadableAddress(Placemark place) {
    ...
    addressParts = addressParts.where((part) => part.isNotEmpty).toSet().toList();
    ...
    try {
      ...
      try {
        ...
        for (Placemark place in placemarks2) {
          if (place.locality?.isNotEmpty == true || place.administrativeArea?.isNotEmpty == true) {
            List<String> parts = [];
            if (place.locality?.isNotEmpty == true) parts.add(place.locality!);
            if (place.administrativeArea?.isNotEmpty == true) parts.add(place.administrativeArea!);
            if (place.country?.isNotEmpty == true) parts.add(place.country!);
  ...
  Future<void> _getCurrentLocation() async {
    ...
    showDialog(
      ...
      if (permission == LocationPermission.deniedForever) {
        if (Navigator.canPop(context)) Navigator.pop(context); // Close loading dialog
        CommonWidget.errorShowSnackBarFor(context, 'Location permissions are permanently denied');
        return;
      }
      ...
      if (Navigator.canPop(context)) Navigator.pop(context); // Close loading dialog
      CommonWidget.successShowSnackBarFor(context, 'Location updated successfully!');
      
    } catch (e) {
      if (Navigator.canPop(context)) Navigator.pop(context); // Close loading dialog
      CommonWidget.errorShowSnackBarFor(context, 'Error getting location: ${e.toString()}');
    }
  }
              ...
              items: dropdownItems.map((String option) {
                return DropdownMenuItem<String>(
                  ...
                  child: Row(
                    children: [
                      Icon(
                        option == "Custom hours" ? Icons.settings : 
                        option.contains(":") ? Icons.schedule : Icons.access_time, 
                      ...
                      Expanded(
                        child: Text(
                          ...
                          style: TextStyle(
                            fontSize: 16,
                            color: option == "Custom hours" ? Colors.orange : Colors.black87,
                          ),
                        ),
                      ),
                    ],
        ...
        children: [
          ...
          Expanded(
            child: ElevatedButton(
              onPressed: _currentStep == _totalSteps - 1 ? _saveShopDetails : _nextStep,
              style: ElevatedButton.styleFrom(
                backgroundColor: ColorClass.base_color,
                padding: const EdgeInsets.symmetric(vertical: 16),
                shape: RoundedRectangleBorder(
    ...
    switch (_currentStep) {
      ...
      case 1:
        if (_shopNameController.text.isEmpty || _shopAddressController.text.isEmpty) {
          CommonWidget.errorShowSnackBarFor(context, "Please fill in business name and address");
          return false;
        }
        if (_currentLat == 0.0 && _currentLng == 0.0) {
          CommonWidget.errorShowSnackBarFor(context, "Please select a valid address with coordinates");
          return false;
        }
        return true;
      case 2:
  ...
  void _saveShopDetails() async {
    ...
    try {
      ...
      if (_selectedOperatingHours == "Custom hours" || _selectedOperatingHours.contains(":")) {
        // Use custom times
      ...
      if (apiAddress.contains("Lat:") || apiAddress.contains("Current Location") || apiAddress.contains("GPS Coordinates")) {
        try {
          // Try to get a better address from coordinates
          String betterAddress = await _getAddressFromCoordinates(_currentLat, _currentLng);
          if (!betterAddress.contains("Lat:") && !betterAddress.contains("Current Location") && !betterAddress.contains("GPS Coordinates")) {
            apiAddress = betterAddress;
          } else {
            // If we still can't get a good address, create a simple one
            apiAddress = 'Business Location, GPS Coordinates (${_currentLat.toStringAsFixed(4)}, ${_currentLng.toStringAsFixed(4)})';
          }
        } catch (e) {
      ...
      var response = await addShopDataManager!.captureVendor(
        ...
        _shopNameController.text, // title
        "Business description", // about
        _selectedCapacity, // time slot
        "0", // price
        _selectedServiceDuration, // duration
        _selectedBusinessType, // category name
        "6717403221480d697d60d26d", // category ID
        [], // detail images
        "", // cover image
        _selectedDays, // working days
        _currentLng, // longitude
        _currentLat, // latitude
        apiAddress, // address name - now guaranteed to be readable
        context,
      );
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        // Success
      CommonWidget.successShowSnackBarFor(context, "Shop details saved successfully!");
      
        // Navigate to shop listing/service details screen
              ...
              ElevatedButton(
                onPressed: () {
                  ...
                  Navigator.pushAndRemoveUntil(
                    context,
                    MaterialPageRoute(builder: (context) => LoginActivity("Login")),
        ...
        try {
          ...
          final errorMessage = responseData['message'] ?? 'Failed to save shop details';
          CommonWidget.errorShowSnackBarFor(context, errorMessage);
        } catch (e) {
          CommonWidget.errorShowSnackBarFor(context, "Failed to save shop details. Please try again.");
        }
      }
