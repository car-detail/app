...
| 1.3 | Extract business logic out of the largest Activities into services/notifiers, one screen at a time, starting with `simple_add_shop_activity.dart` (chosen over the other two: it's one linear form flow, not a hub screen, and directly sets up Phase 2) | app | 🟡 In progress, one safe step done: extracted 3 self-contained UI helpers (`ShopInputField`, `ShopDropdownField`, `ShopDefaultPreviewRow`) into `widgets/shop_form_fields.dart`. 3149 → 2992 LOC. `flutter analyze`: 0 errors. **Needs manual verification: please run the app and confirm the vendor onboarding form (business info step + operating details step) still looks/works correctly before the next extraction step.** Remaining ~30 methods in this file touch shared state and need more careful one-at-a-time extraction. `profile_vendor_list_activity.dart` and `home_activity.dart` not started |
